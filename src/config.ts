...
const schema = z.object({
  ...
  ORGANIZATION_NAME: z.string().trim().min(1).max(100).default("ACME"),
  GOOGLE_CLIENT_ID: z.string().default(""),
  GOOGLE_CLIENT_SECRET: z.string().default(""),
  GOOGLE_REDIRECT_URI: z.string().url().default("http://127.0.0.1:3000/auth/google/callback"),
  GOOGLE_DRIVE_ROOT_FOLDER_NAME: z.string().min(1).default("ACME - Client Files"),
  ...
  WHATSAPP_DEVICE_NAME: z.string().trim().min(1).max(100).default("ACME Client Intake"),
