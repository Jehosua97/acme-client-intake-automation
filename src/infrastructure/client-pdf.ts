...
function reportSections(data: ClientPdfData): ReportSection[] {
  ...
  for (const field of data.fields) {
    ...
    items.push({ label: field.label, value: answerText(answer), ...(answer ? { status: ANSWER_STATUS[answer.status] } : {}) });
  ...
  for (const [fieldId, answer] of Object.entries(data.answers)) {
    ...
    const label = INTERNAL_LABELS[fieldId] ?? (fieldId.match(/^employment\.\d+\.country$/) ? "País de actividad" : null);
  ...
  sections.push({
    ...
    items: data.documents.length
      ? data.documents.map((document) => ({ label: document.name, value: `${(document.size / 1024 / 1024).toFixed(2)} MB` }))
  ...
  for (const section of reportSections(data)) {
    ...
    for (let index = 0; index < section.items.length; index += columns) {
      ...
      for (const [column, item] of row.entries()) {
        ...
        doc.fillColor(isMissing ? "#9a6a22" : "#18231f").font("Helvetica").fontSize(8).text(item.value, x + 6, valueY, { width: columnWidth - 12 });
